...
<html>
	<head>
		...
		$(document).ready(function(){
			...
			async:true,
			data:{"id":id},
			url:"${path}/window/findAllWindow2",
			success:function(result){
				$("#combobox").combobox({valueField:'id',textField: 'text'});
				$("#combobox").combobox('loadData',result.deptVo);
				var win=eval('('+result.windowList+')');
				console.log(win+"----"+result);
				/* if(w!=null){
				var len=w.length;
				for(var i=0;i<len;i++){
				$("#combobox").combobox('select',windows[i].id);
				}
				} */
				}
			});
		...
		<form name="illegalForm" id="illegalForm"  method="post"  enctype="multipart/form-data">
						...
						<tr>
								<td colspan="4" class="title_td">
									<div class="title_div">
										<div class="head_td">编辑违章类型信息</div>
									</div>
								</td>
							</tr>
							<tr>
							 ...
							 <tr >
								<td ><label>违章类型名称：</label><input class="easyui-textbox easyui-validatebox" name="illegalName" id="illegalName" value="${resultSet.illegals.illegalName}" data-options="panelHeight:'auto',required:true"></td>
								<td style="padding-left: 28px;">排序 ：<input class="easyui-textbox easyui-validatebox" name="sort" value="${resultSet.illegals.sort}" id="sort"  data-options="panelHeight:'auto',required:true"></td>
							</tr> 
							<tr>
								<td style="padding-left: 42px;" >描述 ：<input class="easyui-textbox easyui-validatebox" name="illegalType" id="illegalType" value="${resultSet.illegals.illegalType}"  data-options="panelHeight:'auto',required:true"></td>
							</tr>
							<tr style="margin-top: 25px;">&nbsp;</tr>
							<tr>
								<td style="padding-left: 25px;" >是否启用：
								<c:if test="${resultSet.illegals.illegalStatus==1}">
								<button id="enable" type="button" class="open2" style="padding-top: 6px;"></button>
								...
								<c:if test="${resultSet.illegals.illegalStatus==0}">
								<button id="enable" type="button" class="close2" style="padding-top: 6px;"></button>
			               			<input class="easyui-textbox easyui-validatebox" type="hidden" name="illegalStatus" id="illegalStatus" value="0" data-options="panelHeight:'auto',required:true">
								</c:if>
								</td>
								<td style="width:600px;">公众号配置：
								<c:if test="${resultSet.illegals.searchStatus==1}">
								<label>查询违章<button id="searchs" class="open2" type="button" style="padding-top: 5px;"></button></label> 
									<input id="searchStatus" type="hidden" name="searchStatus" value="1"/></br>
								</c:if>
								<c:if test="${resultSet.illegals.searchStatus==0}">
								<label>查询违章<button id="searchs" class="close2" type="button" style="padding-top: 5px;"></button></label> 
								<input id="searchStatus" type="hidden" name="searchStatus" value="0"/></br>
								</c:if>
									<c:if test="${resultSet.illegals.windowStatus==1}">
									 <label>固定窗口<button id="window" class="open2" type="button" style="padding-top: 5px;"></button></label>
									 <input id="windowStatus" type="hidden" name="windowStatus" value="1"/></br>
									</c:if>
									<c:if test="${resultSet.illegals.windowStatus==0}">
									 <label>固定窗口<button id="window" class="close2" type="button" style="padding-top: 5px;"></button></label>
									 <input id="windowStatus" type="hidden" name="windowStatus" value="0"/></br>
									</c:if>
									<c:if test="${resultSet.illegals.adDataStatus==1}">
									 <label>补充资料<button id="addData" class="open2" type="button" style="padding-top: 5px;"></button></label>
									 <input id="adDataStatus" type="hidden" name="adDataStatus" value="1"/></br>
									</c:if>
									<c:if test="${resultSet.illegals.adDataStatus==0}">
									 <label>补充资料<button id="addData" class="close2" type="button" style="padding-top: 5px;"></button></label>
									 <input id="adDataStatus" type="hidden" name="adDataStatus" value="0"/></br>
									</c:if>
						...
						<table class="form_table">
							...
							<tr>
								...
								<textarea class="easyui-textbox easyui-validatebox" name="tips" id="tips" required="true" data-options="validType:['length[2,3000]']" style="width:500px!important;height:100px;overflow-y:hidden;">${resultSet.illegals.tips}</textarea>
								</td>
							</tr>
						</table>
							...
							<tr>
								...
								<td>
								 <div class="easyui-panel" style="width:700px;height:100px;padding:10px;">
											...
											<tr><td><input type='text' name='files' value="${files.originalName }"/><button type='button' id='delBtn' onclick='deleteCurrentRow(this);'/></td></tr>
												</c:forEach>
											</c:when>
											</c:choose>
											</table>
						...
						<table class="form_table" id="searchtable">
							...
							<tr>
								...
								<td>
											...
											<tr><td><input id='filed' name='filed' value="${fields.fieldZname },${fields.fieldEname }"/><button type='button' id='delBtn' onclick='deleteCurrentRow2(this);'/></td></tr>
											</c:forEach>
											</c:when>
											</c:choose>
											</table>
								...
								<td>
										...
										<div>
										<input style="width:220px;height:30px;" id="dataSources" name="dataSources" value="${resultSet.dataList.datasourceZname },${resultSet.dataList.datasourceEname }"/>
										</div>
										</c:if>
										</ul> 
							...
							<%-- <tr>
								...
								<c:choose>
									<c:when test="${resultSet.windowList.size()>0 }">
										...
										<tr><td>
										<input name="windowName" value="${resultSet.windowList.windowName }"/>
											<button type='button' id='delBtn' onclick='deleteCurrentRow2(this);'>
											</button>
											</td></tr>
										</c:forEach>
									</c:when>
										...
										<!-- <ul id="afields">
											...
											<table id="allAfields" >
												<c:choose>
													...
													<tr><td><input id='addField' name='addField' value="${addFields.addZname },${addFields.addEname }"/><button type='button' id='delBtn' onclick='deleteCurrentRow2(this);'/></td></tr>
												</c:forEach>
												</c:when>
												</c:choose>
											</table>
...
<!-- 增加字段表单 -->
	<div id="addSDialog" class="easyui-dialog" closed="true" buttons="#dlg-buttons" title="标题" style="width:600px;height:350px;">
		<input type="checkbox" name="field" value="车牌号,CAR_NUM">车牌号,CAR_NUM</input></br>
		<input type="checkbox" name="field" value="通知书号码,NOTICE_NUMBER">通知书号码,NOTICE_NUMBER</input></br>
		<input type="checkbox" name="field" value="违法时间,ILLEGAL_TIME">违法时间,ILLEGAL_TIME</input></br>
		<input type="checkbox" name="field" value="协查验证码,IDENTITY_CODE">协查验证码,IDENTITY_CODE</input></br>
	</div> 
    <div id="dlg-buttons">
